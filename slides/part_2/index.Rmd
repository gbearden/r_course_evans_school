...
- We'll use `%>%` when working with vectors
- Datasets  
...
- [R for Stata users](https://www.matthieugomez.com/statar/manipulate-data.html)
...
- Using `select()` from the `tidyverse`
...
- No firm, binding rule for using (or not using) the pipe operator with vectors
  ...
  - Depends on whether you are pulling a variable of tibble that you are processing  
    - `pull() == $`  
- In our class, we will use the `tidverse` grammar when working with vectors for consistency sake and ease of learning
...
How would you create a variable that flags a deficit of hosts for the three `room_types` (let's say "deficit" is defined as fewer than 100 hosts)?
...
There are ways to make working with dates easier
  - Use `lubridate` package
    - Dates always formatted as "Date"
...
- Arrange the dataset by `date` in descending order
