---
title: "Part 4 - Adding tools to your Tidyverse toolbox"
...
- You've put the package name and two colons before a function. What's that about?
  - This is how you use functions from packages that you have not loaded with `library()`.
  - For example, `tidyverse` includes the packages that Hadley Wickham and the RStudio team to be "essential" to tidy data processing and analysis. `scales` and `ggthemes` are excluded, but I like to use functions from the packages in my data visualization. 
  - Hence, `ggthemes::scale_fill_few()` and `scales::percent()`
...
- Submit your code along with both plots
...
ggsave('lat_long.jpeg', device = 'jpeg', width = 9.87, height = 8.32, dpi = 300) #<<
```
...
# Using panels in data visualization
