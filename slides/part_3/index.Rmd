...
- Submit 2 “stakeholder-ready” data visualizations and your code on November 3 or before  
  - The assignment will be added to Canvas
...
- How do you clean up your RStudio environment?  
  - `rm()` 
- Check out [NotebookLM](notebooklm.google.com)  
...
- How to [aggregate data](https://gbearden.github.io/r_course_evans_school/slides/part_2/#58)  
...
# Exercise 1 of 2: Troubleshooting

This code contains 3 mistakes. The code is supposed to report the mean temperatures for Peru and Saudi Arabia before 1900. Find the 3 mistakes and correct them. Feel free to use an AI assistant!  
...
mean(saudi$temp, na.rm = TRUE)
mean(peru$temp, rm.na = TRUE)
```
...
# Exercise 2 of 2: Aggregation

Report the average GMAT scores and number of applicants by major for female applicants only. To complete the task, you'll need to:
  - Subset your data using `filter()` (click [here](https://gbearden.github.io/r_course_evans_school/slides/part_2/#46) for help)  
  - Use `group_by()` and `summarise()` to count applicants and and compute the mean 
...
mba %>% 
  ...
  group_by(major) %>% 
  summarise(gmat_mean = mean(gmat), n = n())
```
...
aes(x = NULL, y = NULL, color = NULL
  ...
  , shape = NULL, size = NULL, linewidth = NULL
  )
```
...
- Pre-built themes vs. custom themes
  - `ggplot2` and `ggthemes` themes ([ggthemes](https://github.com/jrnold/ggthemes))
